...
This is mostly a python package, destined to be used by other applications. By itself, it is quite limited. However, we
provide a small CLI tool that acts as a quite powerful commandline VRT generator.
...
It mostly targets and was tested with spreadsheet data (xls, xslx, ods, csv) but actually should work with most data
sources supported by the OGR drivers
...
- Support plain URL as datasource:
    ...
    - find path to dataset inside an archive
      e.g. https://open-data.s3.fr-par.scw.cloud/bdnb_millesime_2022-10-d/millesime_2022-10-d_dep59/open_data_millesime_2022-10-d_dep59_gpkg.zip)
...
1. _**You will need the GDAL/OGR library and its python bindings**_. It is not listed in the requirements.txt file,
   because the installation depends on having the proper libraries already installed on your computer
...
On a classic linux environment, using pip as the installation tool for your python packages, you will need to install
quite a few build dependencies. Alternatively, you can use conda for a simpler installation (conda installs in advance a
lot of stuff, but indeed makes the install less complicated afterward). Choose one of those:
...
You will have to launch the OSGeo4W setup tool (either from a fresh new install, or look into the OSGeo4W menu, for a
Setup entry). After a few steps, you can select the packages you want to install. You will have to make sure you are
installing
...
Then open the OSGeo4W console and type `pip install ogr2vrt_simple`. It should install without trouble. If there are
troubles, read carefully the error message, you might have missed one package to install using OSGeo4W setup tool.

## Use the CLI

Once installed, you will have the `ogr2vrt_cli` command available. For now, it is limited to only one sub-command,
`generate-vrt`:
...
_**Note**: as in the example above, if you are tapping into a remote URL that has special characters in it (e.g.
parenthesis), you will have to surround the URL with quotes or escape the characters (this is a shell issue, not a
python issue, but an issue that needs to be taken care of anyway)_
...
### Or with Docker
A docker image is available on [dockerhub]( https://hub.docker.com/repository/docker/pigeosolutions/ogr2vrt_simple/).

```bash
# Show the help:
docker run --rm --name gdal pigeosolutions/ogr2vrt_simple:latest ogr2vrt_cli generate-vrt --help
```
If you are running it aganst an URL (remote resource), you can simply run it with:
```bash
docker run --rm --name gdal pigeosolutions/ogr2vrt_simple:latest ogr2vrt_cli generate-vrt https://www.data.gouv.fr/api/1/datasets/r/caf190fa-4bbe-4b0e-957d-c5c97676c9bd
```
If you are running it against a local file, you will need to mount the folder containing the file in the docker container. If you make sure the mount path matches the location of your file, you can keep absolute paths. Otherwise, make sure you set relative paths:
```bash
docker run --rm --name gdal -v ~/tmp/:/mnt/tmp  pigeosolutions/ogr2vrt_simple:latest ogr2vrt_cli generate-vrt /mnt/tmp/donnees-communes-sau.csv
```
...
# If not done yet, configure your poetry credentials:
...
`make docker` will build the docker image with `latest` tag. It will also compute a versioned tag depending on the
status of the git repo. If current rev. matches a tag, this will be the docker image tag. If not, it will use the branch
name + rev. id.

It does not push the image to docker hub (do it manually).